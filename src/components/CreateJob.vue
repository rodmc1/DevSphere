...
const handleSubmit = async () => {
  try {
    formData.id = uuidv4(); // Ensure a new UUID is generated on submit
    await axios.post(jobsEndpoint, formData, {
      headers: { 'Content-Type': 'application/json' }
    });
    showSuccessModal.value = true;
    toast.success('Job Added Successfully!');
    router.push(`/job/${formData.id}`);
  } catch (error) {
      ...
      <div class="bg-white px-6 py-8 mb-4 shadow-md rounded-md border m-4 md:m-0">
        <form @submit.prevent="handleSubmit">
          ...
          <div class="mb-4">
            ...
            <select v-model="formData.type" id="type" class="border rounded w-full py-2 px-3" required>
          ...
          <div class="mb-4">
            ...
            <select id="salary" v-model="formData.salary" class="border rounded w-full py-2 px-3" required>
          ...
          <div class="mb-4">
            ...
            <textarea
              v-model="formData.company.company_description"
              id="company_description"
              class="border rounded w-full py-2 px-3"
              rows="4"
              placeholder="What does your company do?"
              >{{ formData.company.company_description }}</textarea
            >
          </div>
